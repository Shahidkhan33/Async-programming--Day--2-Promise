...
heading.innerHTML = "REST COUNTRIES AND THEIR WEATHER REPORT";
...
//To get all country WEATHER REPORT details from OPENWEATHERMAP API by using async/await and fetch()
 async function weatherReport(){   
   const weather = await fetch (`https://api.openweathermap.org/data/2.5/weather?q=${country.name.common}&appid=d3e506754c7c6054d2a6c7e680b5a02f`);
...
//To create CARD
   ...
   card2.innerHTML = ` <div class="card-header">
     <h4 class="card-text">${country.name.common}</h4>
       ...
       <div class="card-body">
       <h6 class="card-text">Temperature: ${weatherData.main.temp}<span> K</span></h6>
       <h6 class="card-text">Ground-Level: ${weatherData.main.grnd_level}<span> hPa</span></h6>
       <h6 class="card-text">Humidity: ${weatherData.main.humidity}<span> %</span></h6>
       <h6 class="card-text">Pressure: ${weatherData.main.pressure}<span> hPa</span></h6>
       <h6 class="card-text">Sea-Level: ${weatherData.main.sea_level}<span> hPa</span></h6>
       <h6 class="card-text">Temp-Max: ${weatherData.main.temp_max}<span> K</span></h6>
       <h6 class="card-text">Temp-Min: ${weatherData.main.temp_min}<span> K</span></h6>
      </div>`;
   document.getElementById("hidden").innerHTML = ""  //hiding the main container of countries
